...
public:
    ...
    void drawArrows(float scale = 1) {
        // tft.fillRect(arrowAreaX, iconAreaY, arrowAreaW, iconAreaH, bruceConfig.bgColor);// VHNF-original
        // tft.fillRect(
        //     tftWidth - arrowAreaX - arrowAreaW, iconAreaY, arrowAreaW, iconAreaH, bruceConfig.bgColor
        // );// VHNF-original
        tft.fillRect(arrowAreaX - 10, iconAreaY, arrowAreaW, iconAreaH, bruceConfig.bgColor); // VHNF-alt
        tft.fillRect(
            tftWidth - (arrowAreaX - 10) - arrowAreaW, iconAreaY, arrowAreaW, iconAreaH, bruceConfig.bgColor
        ); // VHNF-alt

        int arrowSize = scale * 10;
        int lineWidth = scale * 3;

        // int arrowX = BORDER_PAD_X + 1.5 * arrowSize;// VHNF-original
        // int arrowY = iconCenterY + 1.5 * arrowSize;// VHNF-original
        int arrowX = BORDER_PAD_X + 1 * arrowSize - 5;   // VHNF-alt
        int arrowY = iconCenterY + 1.2 * arrowSize - 10; // VHNF-alt
    ...
    void drawTitle(float scale = 1) {
        // int titleY = iconCenterY + iconAreaH / 2 + FG;// VHNF-original
        int titleY = (iconCenterY + iconAreaH / 2 + FG) + 5; // VHNF-alt
