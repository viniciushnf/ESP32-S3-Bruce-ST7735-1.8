...
Opt_Coord drawOptions(
    ...
    int32_t optionsTopY = tftHeight / 2 - menuSize * (FP * 8 + 4) / 2 - 5;
    tft.drawPixel(0, 0, bruceConfig.bgColor);
    if (firstRender) {
        tft.fillRoundRect(
            tftWidth * 0.10, optionsTopY, tftWidth * 0.8, (FP * 8 + 4) * menuSize + 10, 5, bgcolor
        );
        tft.drawRoundRect(
            tftWidth * 0.10,
            tftHeight / 2 - menuSize * (FP * 8 + 4) / 2 - 5,
            tftWidth * 0.8,
            (FP * 8 + 4) * menuSize + 10,
            5,
            fgcolor
    ...
    tft.setTextColor(fgcolor, bgcolor);
    // tft.setTextSize(FM); // VHNF-original
    tft.setTextSize(FP); // VHNF-alt
    tft.setCursor(tftWidth * 0.10 + 5, tftHeight / 2 - menuSize * (FP * 8 + 4) / 2);
...
void drawSubmenu(int index, std::vector<Option> &options, const char *title) {
    ...
    // int middle_up = middle - (tftHeight - 42) / 3 - FM * LH / 2 + 4; // VHNF-original
    // int middle_down = middle + (tftHeight - 42) / 3 - FM * LH / 2;   // VHNF-original
    int middle_up = middle - (tftHeight - 42) / 3 - FP * LH / 2 + 4; // VHNF-alt
    int middle_down = middle + (tftHeight - 42) / 3 - FP * LH / 2;   // VHNF-alt
    ...
    // VHNF-original
    int selectedTextSize = options[index].label.length() <= tftWidth / (LW * FG) - 1 ? FP : FP; // VHNF-alt
    // tft.setTextSize(selectedTextSize); // VHNF-original
    tft.setTextSize(FP); // VHNF-alt
    tft.setTextColor(bruceConfig.priColor);
    // tft.fillRect(6, middle - FG * LH / 2 - 1, tftWidth - 12, FG * LH + 5, bruceConfig.bgColor);//
    // VHNF-original
    tft.fillRect(1, middle - FP * LH / 2 - 1, tftWidth - 2, FP * LH + 5, bruceConfig.bgColor); // VHNF-alt
    ...
    // tft.fillRect(tftWidth - 5, 0, 5, tftHeight, bruceConfig.bgColor); // VHNF-original
    // tft.fillRect(
    //     tftWidth - 5, index * tftHeight / menuSize, 5, tftHeight / menuSize, bruceConfig.priColor
    // ); // VHNF-original
    tft.fillRect(tftWidth - 4, 21, 3, tftHeight - 22, bruceConfig.bgColor); // VHNF-alt
    tft.fillRect(
        tftWidth - 4,
        (index * (tftHeight - 20) / menuSize) + 20,
        3,
        (tftHeight - 20) / menuSize,
        bruceConfig.priColor
    ); // VHNF-VHNF-alt
    // tft.drawRoundRect(0, 0, tftWidth, tftHeight, 5, bruceConfig.priColor); // VHNF-alt
...
void drawStatusBar() {
    ...
    // if (bat > 0) {// VHNF-original
    if (bat > 0 && false) { // VHNF-alt
    ...
    if (sdcardMounted) {
        tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);
        tft.setTextSize(FP);
        tft.drawString("SD", tftWidth - (bat_margin), 7);
        bat_margin = 20;
        i++;
    } // Indication for SD card on screen
    if (gpsConnected) {
        drawGpsSmall(tftWidth - (bat_margin + 23 * i), 2);
        i++;
    }
    if (WiFi.getMode()) {
        drawWifiSmall(tftWidth - (bat_margin + 23 * i), 2);
        i++;
    } // Draw Wifi Symbol beside battery
    if (isWebUIActive) {
        drawWebUISmall(tftWidth - (bat_margin + 23 * i), 2);
        i++;
    } // Draw Wifi Symbol beside battery
    if (BLEConnected) {
        drawBLESmall(tftWidth - (bat_margin + 23 * i), 2);
        i++;
    } // Draw BLE beside Wifi
    if (isConnectedWireguard) {
        drawWireguardStatus(tftWidth - (bat_margin + 24 * i), 2);
        i++;
    } // Draw Wg bedide BLE, if the others exist, if not, beside battery

    if (bruceConfig.theme.border) {
        // tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, bruceConfig.priColor); // VHNF-original
        // tft.drawLine(5, 25, tftWidth - 6, 25, bruceConfig.priColor);                     // VHNF-original

        tft.drawRoundRect(0, 0, tftWidth, tftHeight, 5, bruceConfig.priColor); // VHNF-alt
        tft.drawLine(0, 20, tftWidth, 20, bruceConfig.priColor);               // VHNF-alt
    }

    // if (clock_set) {// VHNF-original
    if (clock_set && false) {   // VHNF-alt
        ...
        // tft.print("BRUCE " + String(BRUCE_VERSION)); // VHNF-original
        tft.print(String(bat) + "%"); // VHNF-alt
    }
}
...
void printFootnote(String text) {
    tft.setTextSize(FP);
    tft.drawRightString(
        text, (tftWidth - BORDER_PAD_X) + 5, (tftHeight - BORDER_PAD_X - FP * LH) + 5, SMOOTH_FONT
    );
}
...
***************************************************************************************/
#define MAX_ITEMS (int)(tftHeight - 20) / (LH * FP)
