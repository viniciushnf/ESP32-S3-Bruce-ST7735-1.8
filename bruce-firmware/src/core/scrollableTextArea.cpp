...
ScrollableTextArea::ScrollableTextArea(const String &title)
    : firstVisibleLine{0}, _redraw{true}, _title(title), _fontSize(FP), _startX(BORDER_PAD_X / 2),
      _startY(BORDER_PAD_Y), _width(tftWidth - BORDER_PAD_X), _height(tftHeight - BORDER_PAD_X - 10) {

    // : firstVisibleLine{0}, _redraw{true}, _title(title), _fontSize(FP), _startX(BORDER_PAD_X),
    //   _startY(BORDER_PAD_Y), _width(tftWidth - 2 * BORDER_PAD_X), _height(tftHeight - BORDER_PAD_X -
    //   BORDER_PAD_Y) // VHNF-original

    drawMainBorder();

    if (!_title.isEmpty()) {
        // printTitle(_title);// VHNF-original

        tft.setCursor((tftWidth - (_title.length() * FP * LW)) / 2, BORDER_PAD_Y - 5); // VHNF-alt
        tft.setTextColor(bruceConfig.priColor, bruceConfig.bgColor);                   // VHNF-alt
        tft.setTextSize(FP);                                                           // VHNF-alt
        _title.toUpperCase();                                                          // VHNF-alt
        tft.println(_title);                                                           // VHNF-alt
        tft.drawLine(
            0, tft.getCursorY() + 1, tftWidth, tft.getCursorY() + 1, bruceConfig.priColor
        ); // VHNF-alt

        // _startY = tft.getCursorY(); // VHNF-original
        _startY = tft.getCursorY() + 5; // VHNF-alt
        // _height -= (_startY - BORDER_PAD_Y); // VHNF-original
        _height -= (_startY - BORDER_PAD_Y) + 10; // VHNF-alt
    }
...
void ScrollableTextArea::draw(bool force) {
    ...
    // if (firstVisibleLine) {// VHNF-original
    //     _scrollBuffer.drawString("...", 0 + _startX, _startY + yOffset);// VHNF-original
    //     yOffset += _pixelsPerLine;// VHNF-original
    //     lines++;// VHNF-original
    // }// VHNF-original

    int32_t tmpHeight = _height;
    // if there is text below
    // if (linesBuffer.size() - firstVisibleLine >= _maxVisibleLines) {// VHNF-original
    //     _scrollBuffer.drawString("...", 0 + _startX, _startY + _height - _pixelsPerLine);// VHNF-original
    //     tmpHeight -= _pixelsPerLine;// VHNF-original
    //     lines++;// VHNF-original
    // }// VHNF-original
