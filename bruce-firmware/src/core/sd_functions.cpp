...
void viewFile(FS fs, String filepath) {
    ...
    // ScrollableTextArea area = ScrollableTextArea("VIEW FILE");// VHNF-original
    // area.fromFile(file);// VHNF-original
    // file.close();// VHNF-original
    // area.show();// VHNF-original

    // The original function goes this far.

    // To restore the original function, uncomment the lines above and delete the lines below until the end of
    // the function.

    // From here to the end of the function, the modifications are made. // VHNF-alt
    // The code below is responsible for adding the filename to the title.
    ...
    int maxWidth = tftWidth - 6;
    String prefix = "FILE: ";
    String finalName = name + ext;
    if (tft.textWidth(prefix + finalName) > maxWidth) {
        while (name.length() > 1) {
            name.remove(name.length() - 1);
            finalName = name + "~" + ext;
            if (tft.textWidth(prefix + finalName) <= maxWidth) { break; }
        }
    }
    finalName = prefix + finalName;
    ScrollableTextArea area = ScrollableTextArea(finalName); // VHNF-alt
    area.fromFile(file);
    file.close();
    area.show();
}
